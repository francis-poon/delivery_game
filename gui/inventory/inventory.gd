...
func _on_inventory_request(item_name: StringName):
	var item_quantity = 0 if not inventory_count.has(item_name) else inventory_count[item_name]	
	inventory_update.emit(item_name, item_quantity)

func _on_buy(item_name: StringName, buy_quantity: int, cost: int):
	if not inventory_count.has(item_name):
		inventory_count[item_name] = 0
		
	# Also handles odd case of cost = 0
	if buy_quantity * cost <= gold:
		gold -= buy_quantity * cost
		inventory_count[item_name] += buy_quantity
	else:
		var affordable_quantity = int(gold/cost)
		gold -= affordable_quantity * cost
		inventory_count[item_name] += affordable_quantity
	
func _on_sell(item_name: StringName, sell_quantity: int, cost: int):
	if not inventory_count.has(item_name):
		inventory_count[item_name] = 0
		
	var amount_sold = sell_quantity if sell_quantity <= inventory_count[item_name] else inventory_count[item_name]
	gold += amount_sold * cost
	inventory_count[item_name] -= amount_sold
	

# Returns dictionary that holds item ids and count
func get_all_items():
	return inventory_count.duplicate(true)

# Returns the count of item by id
func get_item_count(id):
	if inventory_count.has(id):
		return inventory_count[id]
	return 0

func add_item(id, count):
	if inventory_count.has(id):
		inventory_count[id] += count
	else:
		inventory_count[id] = count
		
	if inventory_count[id] <= 0:
		inventory_count.erase(id)
